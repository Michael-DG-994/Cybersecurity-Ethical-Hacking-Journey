...
def game(nome:str, punteggio_user:int, punteggio_comp=int, godmode=bool): 
    user = scelta_user()
    computer = secrets.choice(SCELTE_VALIDE)
    print(f"user: {user}\nVS.\ncomputer: {computer}")
    if SCELTE[user] == computer or godmode == True:
        risultato = "VINTO :)" 
        punteggio_user += 1
    elif user == computer:
...
while True:
    try:
        ...
        for numero_partite in range(numero_partite):
            punteggio_user, punteggio_comp = game(nome,punteggio_user,punteggio_comp, godmode)
            leaderboard = {
                nome : punteggio_user,
